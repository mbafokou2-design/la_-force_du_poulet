...
async function createOrderWithDeps(req, res, deps) {
    ...
    const sendOrderPush = deps?.sendOrderPush ?? fcm_service_1.dispatchOrderPush;
    ...
    try {
        ...
        try {
            ...
            logger_1.logger.error(CONTEXT, `Rollback failed for table ${table_number}`, rollbackErr);
        }
    ...
    let pushResult = { enabled: false, attempted: 0, sent: 0, failed: 0 };
    try {
        pushResult = await sendOrderPush({ orderId: order.id, tableNumber: table_number, message: smsMessage });
    }
    catch (pushErr) {
        logger_1.logger.error("FCM", `Push dispatch failed for order #${order.id}`, pushErr);
    }
