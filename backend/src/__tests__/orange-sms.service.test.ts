...
describe("OrangeSmsService — OAuth", () => {
  it("token OAuth réussi", async () => {
    ...
    assert.equal(tokenCalls, 1, "le token doit être mis en cache");
  });

  it("token OAuth échoué", async () => {
...
describe("OrangeSmsService — envoi SMS", () => {
  it("SMS réussi", async () => {
    const fetchImpl = makeFetch(async (url) => {
      if (url.includes("/oauth/v3/token")) return jsonResponse(200, TOKEN_JSON);
      if (url.includes("/smsmessaging/")) return jsonResponse(201, SMS_OK_JSON);
      throw new Error(url);
    });
    const service = new OrangeSmsService(CONFIG, fetchImpl as typeof fetch);
  ...
  it("credentials invalides", async () => {
    const fetchImpl = makeFetch(async (url) => {
  ...
  it("retry sur erreur transitoire puis succès", async () => {
    let smsAttempts = 0;
    const fetchImpl = makeFetch(async (url) => {
      if (url.includes("/oauth/v3/token")) return jsonResponse(200, TOKEN_JSON);
      smsAttempts += 1;
  ...
  it("ne retry pas une erreur de crédit", async () => {
    let smsAttempts = 0;
    const fetchImpl = makeFetch(async (url) => {
      if (url.includes("/oauth/v3/token")) return jsonResponse(200, TOKEN_JSON);
      smsAttempts += 1;
